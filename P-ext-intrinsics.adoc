...
The shift amounts for slx and srx are taken modulo the input size.
...
| `int32_t __riscv_merge_i32(int32_t rs1, int32_t rs2, uint32_t rd);`
| `merge`, `mvm`, `mvmn`

| `uint32_t __riscv_merge_u32(uint32_t rs1, uint32_t rs2, uint32_t rd);`
| `merge`, `mvm`, `mvmn`
|===
...
| `int64_t __riscv_merge_i64(int64_t rs1, int64_t rs2, uint64_t rd);`
| `merge`, `mvm`, `mvmn`(RV64), +
  2x `merge`, `mvm`, `mvmn`(RV32)

| `uint64_t __riscv_merge_u64(uint64_t rs1, uint64_t rs2, uint64_t rd);`
| `merge`, `mvm`, `mvmn`(RV64), +
  2x `merge`, `mvm`, `mvmn`(RV32)
|===
...
<<<
=== Saturating and Rounding Shift

Compiler will choose an immediate form when possible.

Shift amounts are a signed value taken from the lower 8 bits. Negative shift
amounts shift right. Non-negative shift amounts shift left.
...
| `int32_t __riscv_ssha_i32(int32_t rs1, int shamt);`
| `ssha`, `sslai`, `srai`(RV32), +
  `pssha.ws`, `psslai.w`, `psrai.w`(RV64)

| `int32_t __riscv_sshar_i32(int32_t rs1, int shamt);`
| `sshar`, `sslai`, `srari`(RV32), +
  `psshar.ws`, `psslai.w`, `psrari.w`(RV64)

| `uint32_t __riscv_sshl_u32(uint32_t rs1, int shamt);`
| `sshl`(RV32), +
  `psshl.ws`(RV64)

| `uint32_t __riscv_sshlr_u32(uint32_t rs1, int shamt);`
| `sshlr`(RV32), +
  `psshlr.ws`(RV64)
|===
...
| `int64_t __riscv_sha_i64(int64_t rs1, int shamt);`
| `sha`, `slli`, `srai`

| `int64_t __riscv_shar_i64(int64_t rs1, int shamt);`
| `shar`, `slli`, `srari`

| `uint64_t __riscv_shl_u64(uint64_t rs1, int shamt);`
| `shl`

| `uint64_t __riscv_shlr_u64(uint64_t rs1, int shamt);`
| `shlr`
|===
...
Shift amounts are taken modulo the input size.
...
All shift amounts are taken modulo 32. If the shift amount exceeds the element
size, the result is all zeros or all sign bits.
...
<<<
=== Packed Saturating and Rounding Shift
...
All shift amounts are taken modulo 32. If the shift amount exceeds the result
element size, the result is all zeros or all sign bits.
...
All shift amounts are taken modulo 32. If the shift amount exceeds the input
element size, the result is all zeros or all sign bits.
...
l|
int8x4_t
__riscv_pnclipp_i8x4(int16x2_t rs1, int16x2_t rs2);
| `mv`\+`mv`+`pnclipi.b`(RV32), +
  `pnclipp.b`(RV64)

l|
uint8x4_t
__riscv_pnclipup_u8x4(uint16x2_t rs1, uint16x2_t rs2);
| `mv`\+`mv`+`pnclipiu.b`(RV32), +
  `pnclipup.b`(RV64)

l|
int16x2_t
__riscv_pnclipp_i16x2(int32_t rs1, int32_t rs2);
| `mv`\+`mv`+`pnclipi.h`(RV32), +
  `pnclipp.h`(RV64)

l|
uint16x2_t
__riscv_pnclipup_u16x2(uint32_t rs1, uint32_t rs2);
| `mv`\+`mv`+`pnclipiu.h`(RV32), +
  `pnclipup.h`(RV64)
|===
...
l|
int8x8_t
__riscv_pnclipp_i8x8(int16x4_t rs1, int16x4_t rs2);
| `pnclipp.b`(RV64), +
  2x `pnclipi.b`(RV32)

l|
uint8x8_t
__riscv_pnclipup_u8x8(uint16x4_t rs1, uint16x4_t rs2);
| `pnclipup.b`(RV64), +
  2x `pnclipiu.b`(RV32)

l|
int16x4_t
__riscv_pnclipp_i16x4(int32x2_t rs1, int32x2_t rs2);
| `pnclipp.h`(RV64), +
  2x `pnclipi.h`(RV32)

l|
uint16x4_t
__riscv_pnclipup_u16x4(uint32x2_t rs1, uint32x2_t rs2);
| `pnclipup.h`(RV64), +
  2x `pnclipiu.h`(RV32)

l|
int32x2_t
__riscv_pnclipp_i32x2(int64_t rs1, int64_t rs2);
| `pnclipp.w`(RV64), +
  2x `nclipi`(RV32)

l|
uint32x2_t
__riscv_pnclipup_u32x2(uint64_t rs1, uint64_t rs2);
| `pnclipup.w`(RV64), +
  2x `nclipiu`(RV32)
|===
...
Reverse the order of elements in a packed vector.
...
l|
int8x4_t __riscv_prev_i8x4(int8x4_t rs1);
| `rev8`(RV32), +
  `rev8`+`srli`(RV64)

l|
uint8x4_t __riscv_prev_u8x4(uint8x4_t rs1);
| `rev8`(RV32), +
  `rev8`+`srli`(RV64)

l|
int16x2_t __riscv_prev_i16x2(int16x2_t rs1);
| `ppairoe.h`

l|
uint16x2_t __riscv_prev_u16x2(uint16x2_t rs1);
| `ppairoe.h`
|===
...
l|
int8x8_t __riscv_prev_i8x8(int8x8_t rs1);
| `rev8`(RV64), +
  2x `rev8`(RV32)

l|
uint8x8_t __riscv_prev_u8x8(uint8x8_t rs1);
| `rev8`(RV64), +
  2x `rev8`(RV32)

l|
int16x4_t __riscv_prev_i16x4(int16x4_t rs1);
| `rev16`(RV64), +
  2x `ppairoe.h`(RV32)

l|
uint16x4_t __riscv_prev_u16x4(uint16x4_t rs1);
| `rev16`(RV64), +
  2x `ppairoe.h`(RV32)

l|
int32x2_t __riscv_prev_i32x2(int32x2_t rs1);
| `ppairoe.w`(RV64), +
  2x `mv`(RV32)

l|
uint32x2_t __riscv_prev_u32x2(uint32x2_t rs1);
| `ppairoe.w`(RV64), +
  2x `mv`(RV32)
|===
...
l|
uint64_t
__riscv_mulu_w00_u64(uint32x2_t rs1, uint32x2_t rs2);
| `mulu.w00`(RV64), +
  `wmulu`(RV32)

l|
uint64_t
__riscv_mulu_w01_u64(uint32x2_t rs1, uint32x2_t rs2);
| `mulu.w01`(RV64), +
  `wmulu`(RV32)

l|
uint64_t
__riscv_mulu_w11_u64(uint32x2_t rs1, uint32x2_t rs2);
| `mulu.w11`(RV64), +
  `wmulu`(RV32)
...
__riscv_pwmaccsu_i32x2(int32x2_t rd,
                       ...
                       uint16x2_t rs2);
| `pwmaccsu.h`(RV32), +
  `pwcvtu.wh`\+`pwcvtu.wh`+`pmaccsu.w.h00`(RV64)
|===
...
These intrinsics allow the pointer to have arbitrary alignment rather than
requiring strict element, word, or dword alignment. The P extension recommends
implementations support efficient misaligned GPR loads, but does not
require it. Compilers may split these intrinsics into multiple loads if
the target CPU or profile is not known to support misaligned accesses and the
compiler cannot be prove the access is aligned. Compiler hints like
`__builtin_assume_aligned` may help.
...
These intrinsics allow the pointer to have arbitrary alignment rather than
requiring strict element, word, or dword alignment. The P extension recommends
implementations support efficient misaligned GPR stores, but does not
require it. Compilers may split these intrinsics into multiple stores if
the target CPU or profile is not known to support misaligned accesses and the
compiler cannot be prove the access is aligned. Compiler hints like
`__builtin_assume_aligned` may help.
...
For a packed type with `N` elements of `E` bits each, the effective slide amount
is `n = rs2 & (N - 1)` elements, or `k = n * E` bits. A zero effective amount
returns `rd` unchanged. Signed and unsigned forms perform the same bit movement.

Slide up moves elements of `rd` toward the most-significant end, filling the
vacated low elements from the most-significant `n` elements of `rs1`. Slide down
moves elements toward the least-significant end, filling the vacated high
elements from the least-significant `n` elements of `rs1`.

==== 32-bit

On RV64, the bit shift amount must be masked to 5 bits: the hardware shifts
otherwise use 6 bits. The following example sequences use `dst` for the original
`rd` and the result, `src` for `rs1`, and `count` for `rs2`. `LOG2_E` is 3 for
byte elements and 4 for halfword elements. Temporaries do not overlap the inputs
or destination; only the low 32 bits of the result belong to the packed value.

.RV64 slide up of a 32-bit packed value
[source,asm]
----
slli t0, count, LOG2_E
andi t0, t0, 31
slli t1, src, 32
slx dst, t1, t0
----

.RV64 slide down of a 32-bit packed value
[source,asm]
----
slli t0, count, LOG2_E
andi t0, t0, 31
...
On RV32, the mappings require the
<<packed-slide-rv32-lowering,word selection described below>> in addition to
the listed shifts.
...
__riscv_pslideupx_i8x8(int8x8_t rd, int8x8_t rs1,
                       unsigned int rs2);
| `slli`\+`slx`(RV64), +
  `slli` + word selection + `slx` + `slx`(RV32)
...
__riscv_pslidedownx_i8x8(int8x8_t rd, int8x8_t rs1,
                         unsigned int rs2);
| `slli`\+`srx`(RV64), +
  `slli` + word selection + `srx` + `srx`(RV32)
...
[[packed-slide-rv32-lowering]]
===== RV32 word selection

The RV32 entries above summarize the core operations, not a complete instruction
sequence. Scaling the element count gives a bit count whose low 6 bits are `k`.
RV32 `slx` and `srx` use only the low 5 bits, `r = k & 31`. Bit 5 must also be
tested to select the source words for slides crossing the 32-bit boundary.

Let `Dhi`, `Dlo` and `Shi`, `Slo` be the original high and low 32-bit words of
`rd` and `rs1`, respectively. In the table below, `SLX32(a, b, r)` means the result
of an RV32 `slx` with its destination initialized to `a`, its first source set
to `b`, and shift count `r`; `SRX32` is defined analogously for `srx`.
...
Word selection requires a branch or equivalent conditional selection, plus any
moves needed to initialize the destinations. Original source words must be
preserved until both result words have been computed. For `k = 32`, `r = 0`:
slide up returns `Dlo` as the high word and `Shi` as the low word; slide down
returns `Slo` as the high word and `Dhi` as the low word. For 32-bit elements,
`k` is either 0 or 32, so only word selection is needed and the two shifts can
be omitted. Constant slide amounts can also simplify these sequences.
